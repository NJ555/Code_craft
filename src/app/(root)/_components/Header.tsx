...
import RunButton from "./RunButton";
import HeaderProfileBtn from "./HeaderProfileBtn";
import { fetchQuery, fetchMutation } from "convex/nextjs"; // ✅ safer for server
...
async function Header() {
  ...
  if (user) {
    try {
      // ✅ Upsert user into Convex safely
      ...
      // ✅ Fetch convex user (safe)
      convexUser = await fetchQuery(api.users.getUser, {
        userId: user.id,
      });
    } catch (err) {
